...
for file in glob.glob("*.mid"):
    ...
    for note in pm.instruments[0].notes: # Append the notes to the piano instrument
        ...
        if end <= on1 and end <= on2 and end <= on3: # Skip this note
            continue
...
os.chdir("../piano_poly3_midi") # Switch to the monophonic midi folder directory 
...
for zz, midi in enumerate(out_midis):
    ...
    notes = np.full([length-1, 3], -1, dtype=np.int) # Pitch values per time frame
