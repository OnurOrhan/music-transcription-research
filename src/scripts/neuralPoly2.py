...
BINS = 88 # Number of frequency bins
...
def sample(filename):
    ...
    with open("%s.txt" % filename) as f:
        for line in f:
            (i1, i2, i3, i4) = line.split()
            num_samples += abs(int(float(i4)*Sr/512) - int(float(i3)*Sr/512))
    ...
    with open("%s.txt" % filename) as f:
        for line in f:
            (i1, i2, i3, i4) = line.split()
            (a, b, c, d) = (midinote2Index(int(i1)), midinote2Index(int(i2)), float(i3), float(i4))
            
            y = np.array(Spec[int(c*Sr/512):int(d*Sr/512), :])
            length = len(y)
            ymin = y.min(axis=1)
            ymax = y.max(axis=1)
            ...
            pV[sample:sample+length] = np.tile(oneHotVector([a,b]), (length, 1))
            sample += length
...
def saveData():
    print("Saving data...")
    np.save("../data/2tx.npy", trainx)
    np.save("../data/2ty.npy", trainy)
    np.save("../data/2vx.npy", validx)
    np.save("../data/2vy.npy", validy)
    print("Data saved...")

def loadData():
    global trainx, trainy, validx, validy
    print("Loading data...")
    trainx = np.load("../data/2tx.npy")
    trainy = np.load("../data/2ty.npy")
    validx = np.load("../data/2vx.npy")
    validy = np.load("../data/2vy.npy")
    print("Data loaded...")
...
def modelInit(dropouts=[0.15, 0.15, 0.15]): # Initialize the network model
    global model
    if model is None:
        ...
        b = Dense(BINS*3, activation="sigmoid", name="dense3")(b)
        b = Dropout(dropouts[2], name="dropout3")(b)
...
def plotModel():
    ...
    if model is not None:
        plot_model(model, to_file='../figures/model2.png')
